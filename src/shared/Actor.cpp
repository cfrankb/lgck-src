...
void CActor::attackPlayer()
{
    ...
    if (!player.getState(CHitData::STATE_HIT)
        && !player.getState(CHitData::STATE_DEAD)
        && ((!protoActor.m_nPowerLevel) ||
...
void CActor::managePath()
{
    ...
    case CGame::DOWN:
        bcanWalk = (m_nY + frame.m_nHei < m_game->BUFFERHEI)
                   && bcanWalk;
        break;
    ...
    case CGame::RIGHT:
        bcanWalk = (m_nX + frame.m_nLen < m_game->BUFFERLEN)
                   && bcanWalk;
        break;
...
void CActor::autoCenter()
{
    ...
    if (rx < m_game->m_screenLen / 8
           && m_game->_mx() > 0) {
        autoCenter(CGame::LEFT);
        rx = m_nX - m_game->_mx();
    }

    if ((rx > m_game->m_screenLen - m_game->m_screenLen /8)
        && (m_game->_mx() < m_game->BUFFERLEN - m_game->m_screenLen)   ){
        autoCenter(CGame::RIGHT);
        rx = m_nX - m_game->_mx();
    }
    ...
    for (int i=0; i < map.m_fwCount; ++i) {
        ...
        if (map.m_nFwClass[i] == CLASS_OPEN_TO_OWNER) {
            if (!protoActor.isPlayer() || aim == CGame::FALL) {
                ...
                if (!protoObject.m_nProtoBuddy || !inventory || !inventory->hasItem( protoObject.m_nProtoBuddy )
                    ||  protoObject.isStateSolid(aim)) {
                    return true;
                }
            }
        }
        ...
        if (protoObject.m_solidState & CProto::SOLID_OVERIDE) {
            ...
            if ((map.m_nFwClass[i] == CLASS_SOLID_ANIMATED )
                    || (map.m_nFwClass[i] == CLASS_TELEPORT_PAD )
                    || (map.m_nFwClass[i] == CLASS_TELEPORT_DESTINATION )) {
                return true;
            }
        }
    }
